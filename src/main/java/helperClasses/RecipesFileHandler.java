...
public class RecipesFileHandler {
    public static HashMap<List<Hashtable<Liquid,Double>>,List<Double>> loadRecipes() {
        ...
        HashMap<List<Hashtable<Liquid,Double>>,List<Double>> recipes = new HashMap<>();
        for(int i = 0; i < recipe_mixes.size(); i++) {
            recipes.put(recipe_mixes.get(i), recipes_rates.get(i));
        }
